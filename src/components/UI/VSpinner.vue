...
const spinnerElement = ref<HTMLElement>()
const cssProperties = ref({})
const resizeInterval = ref<ReturnType<typeof setInterval>>()
onMounted(() => {
  init()
...
const init = () => {
  const domElement = spinnerElement.value?.parentNode
  ...
  Object.assign(
    ...
    props.fullScreen 
      ? FULL_SCREEN_STYLE
      : calculateSpinnerStyle(domElement as HTMLElement)
  )
}

const calculateSpinnerStyle = (element: HTMLElement) => {
